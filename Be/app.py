...
# endpoint creat data listrik
@view_config(route_name='create-data', request_method='POST', renderer="json")
def listrik_create(request):
    # create data sembako
    auth_user = auth_jwt_verify(request)
    if auth_user:
        with connection.cursor() as cursor:
            sql = "INSERT INTO listrik (nama, harga, ketersediaan) VALUES (%s, %s, %s)"
...
def listrik_update(request):
    # update data token listrik
    auth_user = auth_jwt_verify(request)
    if auth_user:
        with connection.cursor() as cursor:
            sql = "UPDATE listrik SET nama=%s, harga=%s, ketersediaan=%s WHERE id=%s"
...
def listrik_delete(request):
    ...
    if auth_user:
        with connection.cursor() as cursor:
            ...
            sql = "DELETE FROM listrik WHERE id=%s"
